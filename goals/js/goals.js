var user;
            ...
            ref.orderByChild("status").equalTo("goal").on("child_added", function(snapshot, prevChildKey) {
                ...
                if(user==newGoal.user){
                    $("#goals").append('<div class="col-sm-4 col-lg-4 col-md-4" id="'+snapshotkey+'"><div class="thumbnail"><div class="caption"><h4 class="pull-right">'+newGoal.deadline+'</h4><h4 onclick="goalFn(\''+snapshotkey+'\');"><a>'+newGoal.title+'</a></h4><p>'+newGoal.instructions+'<span onclick="goalFn(\''+snapshotkey+'\');"><a><font size="4">[...]</font></a></span></p></div><div class="ratings"><p class="pull-right"><a href="" style="color:#0B610B">'+newGoal.completions+' completions</a></p><p><button id="'+snapshotkey+'Btn" type="button" class="btn btn-default" onclick="myFunction(\''+snapshotkey+'\',url,deadline,title,instructions,completions)">Complete</button></p></div></div></div>'); console.log("_____");
                }
            });
